...
!** Date: 11/17/2021
!**
!** Title: 2x2 Polarize Square Model with a Streched Dipole
!** 
...
!**
!**          The dipoles in this model are streched, which
!**          means that rather than being placed in the center
!**          of hard spheres, they are located slightly outside
!**          of the center. In this case, the dipole is 1.5 times
!**          longer along the axis that the dipole points.
!**
!**          This simulation incorperated cell + neighbor list,
!**          beinary tree, and false positioning DMD efficiency
!**          techniques.  
!**
!*************************************************************
...
integer, parameter :: mer = 6 ! number of hardspheres which make one cube (4 hard spheres + 2 polarized spheres)
...
real(kind=dbl), parameter :: sigma2 = 0.85 * sigma1 ! distance from sphere center to second discontinuity
real(kind=dbl), parameter :: sigma3 = 0.95 * sigma1 ! distance from sphere center to third discontinuity
real(kind=dbl), parameter :: sigma4 = 1.15 * sigma1 ! distance from sphere center to fourth discontinuity
real(kind=dbl), parameter :: sigma5 = 1.4 * sigma1 ! distance from sphere center to fifth discontinuity
real(kind=dbl), parameter :: epsilon1 = 1.0000 ! reduced energy parameter
real(kind=dbl), parameter :: epsilon2 = 2.5426 * epsilon1 ! depth of first well
real(kind=dbl), parameter :: epsilon3 = 1.5916 * epsilon1 ! depth of second well 
real(kind=dbl), parameter :: epsilon4 = 0.8259 * epsilon1 ! depth of third well
real(kind=dbl), parameter :: epsilon5 = 0.3146 * epsilon1 ! depth of fourth well
...
real(kind=dbl), parameter :: sg5sq = sigma5 ** 2
...
real(kind=dbl), parameter :: ipol1bond = (sigma1 / 8.) - delta ! inner first polarized bond
real(kind=dbl), parameter :: opol1bond = (sigma1 / 8.) + delta ! outer first polarized bond
real(kind=dbl), parameter :: ipol2bond = (9. * sigma1 / 8.) - delta ! inner second polarized bond
real(kind=dbl), parameter :: opol2bond = (9. * sigma1 / 8.) + delta ! outer second polarized bond
! ** anderson thermostat *************************************
...
real(kind=dbl), parameter :: orderwidth = sigma5


! ** file management *****************************************
character(len=15), parameter :: simtitle = 'polsquare'
character(len=10), parameter :: simid = '2Ea10e05_'
...
real(kind=dbl), parameter :: nbrRadiusMin = (nbrRadiusMinInt / (nbrRadiusMinInt - 1)) * (sigma5 - (sigma1 / nbrRadiusMinInt)) ! minimum required radius
	...
	do ! for every cube
		...
		do m = 1, mer ! for every sphere making up a cube 
			...
			if (m == 1) then ! if the cube is the first in the assembly
                ...
                if (m <= (mer - 2)) then ! if a hard sphere
                ...
                else ! if a polarized sphere
                    if (m == (mer - 1)) then ! if the first polarized sphere
                        ! the location of the first polarized sphere is based on the location of the first hard sphere
                        square(i)%circle(m)%fpos%r(1) = square(i)%circle(1)%fpos%r(1) + (sigma1 / 8.) * cos(theta - halfpi)
                        square(i)%circle(m)%fpos%r(2) = square(i)%circle(1)%fpos%r(2) + (sigma1 / 8.) * sin(theta - halfpi)
                    else if (m == (mer)) then ! if the second polarized sphere
                        square(i)%circle(m)%fpos%r(1) = square(i)%circle(2)%fpos%r(1) + (sigma1 / 8.) * cos(-(theta + halfpi))
                        square(i)%circle(m)%fpos%r(2) = square(i)%circle(2)%fpos%r(2) + (sigma1 / 8.) * sin(theta + halfpi)
                    endif
                endif
                ! apply periodic boundary conditions
                do q = 1, ndim
                    if (square(i)%circle(m)%fpos%r(q) < 0.0) square(i)%circle(m)%fpos%r(q) = &
                        square(i)%circle(m)%fpos%r(q) + region
                    if (square(i)%circle(m)%fpos%r(q) > region) square(i)%circle(m)%fpos%r(q) = &
                        square(i)%circle(m)%fpos%r(q) - region
                end do
			end if 
			...
			if (i>1) then ! if the spheres do not belong to the first cube
				do j = 1, i-1 ! for all already placed squares
					do n = 1, (mer - 2) ! for all hard spheres in each square
			...
			group: do m = 1, mer 
                ...
                if (m <= (mer - 2)) then
                    ...
                    square((x - 1) * cell + y)%circle(m)%fpos%r(2) = center%r(2) + radius * sin(ori + &
                        (real(randori) * halfpi) + (m - 1) * halfpi)
                elseif (m == (mer - 1)) then 
                    square((x - 1) * cell + y)%circle(m)%fpos%r(1) = square((x - 1) * cell + y)%circle(1)%fpos%r(1) + &
                        (sigma1 / 8.) * cos(ori + (real(randori) * halfpi) - (pi / 4.))
                    square((x - 1) * cell + y)%circle(m)%fpos%r(2) = square((x - 1) * cell + y)%circle(1)%fpos%r(2) + &
                        (sigma1 / 8.) * sin(ori + (real(randori) * halfpi) - (pi / 4.))
                elseif (m == mer) then 
                    square((x - 1) * cell + y)%circle(m)%fpos%r(1) = square((x - 1) * cell + y)%circle(2)%fpos%r(1) + &
                        (sigma1 / 8.) * cos(ori + (real(randori) * halfpi) + (3. * pi / 4.))
                    square((x - 1) * cell + y)%circle(m)%fpos%r(2) = square((x - 1) * cell + y)%circle(2)%fpos%r(2) + &
                        (sigma1 / 8.) * sin(-(ori + (real(randori) * halfpi) - (pi / 4.)))
                endif
			end do group
		end do y_axis
...
subroutine set_polarity ()
	...
	do i = 1, cube 
		do m = 1, mer 
			if (((square(i)%chai == 1) .and. (m == (mer - 1))) .or. ((square(i)%chai == 2) .and. (m == mer))) then 
                square(i)%circle(m)%pol = 1 ! positive charge
			else if (((square(i)%chai == 2) .and. (m == (mer - 1))) .or. ((square(i)%chai == 1) .and. (m == mer))) then  
	...
	do j1 = 1, mols 
		...
		compare: do 
			...
			if ((a%one == b%one) .and. (b%two > a%two)) then ! if b is uplist of a and they are on the same group
				...
				else if (pol1bonded (a%two, b%two) .and. pol1overlap (d)) then 
                    check_bond = .true.
                    write (simiounit, 5) a%two, b%two, a%one, d 
                    5 format (" check_boundaries: circles ", I3, " and ", I3, " of square ", I3 &
                        , " are pol1-bonded and overlapping (", F6.2, ").")
                else if (pol2bonded (a%two, b%two) .and. pol2overlap (d)) then 
                    check_bond = .true.
                    write (simiounit, 6) a%two, b%two, a%one, d 
                    6 format (" check_boundaries: circles ", I3, " and ", I3, " of square ", I3 &
                        , " are pol2-bonded and overlapping (", F6.2, ").")
                endif 
			else if ((b%one > a%one) .and. (b%two <= (mer - 2)) .and. (a%two <= (mer - 2))) then ! if b is uplist of a and they not on the same group 
...
logical function neighborbonded (i, j)
	...
	if ((((j-i) == 1) .or. ((j-i) == 3)) .and. (j <= (mer - 2))) then ! if the two spheres are next to one another
        neighborbonded = .true.
	end if 
...
logical function pol1bonded (i, j) 
    implicit none 
    ! ** calling variables ***********************************
    integer, intent (in) :: i, j ! sphere identifiers
    ! ** local variables *************************************
    ...
    pol1bonded = .false. 
    if (((i == 1) .and. (j == 5)) .or. ((i == 2) .and. (j == 6))) then 
        pol1bonded = .true.
    endif
end function pol1bonded

logical function pol2bonded (i, j) 
    implicit none 
    ! ** calling variables ***********************************
    integer, intent (in) :: i, j ! sphere identifiers
    ! ** local variables *************************************
    ...
    pol2bonded = .false. 
    if (((i == 2) .and. (j == 5)) .or. ((i == 1) .and. (j == 6))) then 
        pol2bonded = .true.
    endif
end function pol2bonded
...
logical function pol1overlap(distance)
    implicit none
    real(kind=dbl), intent(in) :: distance
    
    pol1overlap = (distance > (opol1bond + tol)) .or. (distance < (ipol1bond - tol))
end function pol1overlap

logical function pol2overlap(distance)
    implicit none
    real(kind=dbl), intent(in) :: distance
    
    pol2overlap = (distance > (opol2bond + tol)) .or. (distance < (ipol2bond - tol))
end function pol2overlap
...
subroutine open_files ()
    ...
    write(annealiounit,*) 'id, set, actual, total, potential, kinetic, h2ts, ht2o, anti, poly2, full, fulls, fullo,',&
        'poly1aa, poly1abba, poly1bb, poly1, percolation, nclust, nematic'
    ...
    write(reportiounit, *) ' well 3 depth , ', epsilon4
    write(reportiounit, *) ' well 3 length , ', sigma4
    write(reportiounit, *) ' well 3 depth , ', epsilon5
    write(reportiounit, *) ' well 3 length , ', sigma5
    ...
    write(opiounit, *) 'time, events, h2ts, h2ts_fluc, h2to, h2to_fluc, anti, anti_fluc,', &
        ...
        ' poly1abba, poly1abba_fluc, poly1bb, poly1bb_fluc, poly1, poly1_fluc, percolation, nclust, nclust_fluc'
...
subroutine report_properties()
    ...
    write(opiounit, *) timenow, ',', n_events, ',', h2ts%sum, ',', h2ts%sum2, ',', h2to%sum, ',', h2to%sum2, &
        ...
        poly1%sum,',', poly1%sum2, ',', percy%sum, ',', nclust%sum, ',', nclust%sum2
...
subroutine calculate_poperties()
    ...
    ! ** Order Parameters ****************************************
    call accumulate_order (poly1%value, poly1aa%value, poly1abba%value, poly1bb%value, h2ts%value, &
		...
		do m = 1, mer 
			...
			uplist: do 
                ...
                if (i /= j) then 
					...
					else if (rij < sigma3) then ! if the pair is within the second, inner-middle well 
						if ((square(i)%circle(m)%pol * square(j)%circle(n)%pol) == -1) then ! if the spheres are attracted to
                            potential = potential - epsilon3
						else if ((square(i)%circle(m)%pol * square(j)%circle(n)%pol) == 1) then ! if the spheres are repulsed from
                            potential = potential + epsilon3
						end if
					else if (rij < sigma4) then ! the pair is within the third, outer-middle well
						if ((square(i)%circle(m)%pol * square(j)%circle(n)%pol) == -1) then ! if the spheres are attracted to
                            potential = potential - epsilon4
						else if ((square(i)%circle(m)%pol * square(j)%circle(n)%pol) == 1) then ! if the spheres are repulsed from
                            potential = potential + epsilon4
						end if
                    else if (rij < sigma5) then ! the pair is within the fourth, outermost well
                        if ((square(i)%circle(m)%pol * square(j)%circle(n)%pol) == -1) then ! if the spheres are attracted to
                            potential = potential - epsilon5
                        else if ((square(i)%circle(m)%pol * square(j)%circle(n)%pol) == 1) then ! if the spheres are repulsed from
                            potential = potential + epsilon5
                        end if
					end if
...
subroutine accumulate_order(poly1, aa, abba, bb, head2tailsame, head2tailoppo, antiparallel, poly2, &
...
recursive subroutine clusteranal(clust, perc, a) ! #percy
    ...
    real(kind=dbl), parameter :: cluster_dist = sigma5
...
subroutine determine_nematic(nematic)
    ...
    type(position), dimension(mer - 2) :: rcircles ! position of the group of particles making up the square
    type(position) :: dr
...
subroutine predict(a, b)
	...
	if (a%one == b%one) then ! if the particle pair are part of the same group 
        if (pol1bonded (a%two, b%two)) then 
            prediction = pol1bond_event (rij, vij, b%one, b%two)
        else if (pol2bonded (a%two, b%two)) then 
            prediction = pol2bond_event (rij, vij, b%one, b%two)
		else if (neighborbonded (a%two, b%two)) then 
		...
		else if ((square(a%one)%circle(a%two)%pol == 0) .and. (square(b%one)%circle(b%two)%pol == 0)) then
...
type(event) function neighborbond_event (rij, vij, j, n)
	...
	if (bij < 0.) then ! the centers are approaching
		if (idiscr > 0.) then ! the centers will collide 
            neighborbond_event%type = 10 ! an event will occur at the inner bond length
            neighborbond_event%time = (-bij - sqrt(idiscr)) / aij
		else ! the repulsive centers will miss each other 
            neighborbond_event%type = 11 ! an event will take place at the outer bond length
            neighborbond_event%time = (-bij + sqrt(odiscr)) / aij
		end if 
	else ! the centers are receding
        neighborbond_event%type = 11 ! an event will occur at the outer bond length 
        neighborbond_event%time = (-bij + sqrt(odiscr)) / aij
	end if 
...
type(event) function crossbond_event (rij, vij, j, n)
	...
	if (bij < 0.) then ! the centers are approaching
		if (idiscr > 0.) then ! the centers will collide 
            crossbond_event%type = 12 ! an event will occur at the inner bond length
            crossbond_event%time = (-bij - sqrt(idiscr)) / aij
		else ! the repulsive centers will miss each other 
            crossbond_event%type = 13 ! an event will take place at the outer bond length
            crossbond_event%time = (-bij + sqrt(odiscr)) / aij
		end if 
	else ! the centers are receding
        crossbond_event%type = 13 ! an event will occur at the outer bond length 
        crossbond_event%time = (-bij + sqrt(odiscr)) / aij
	end if 
end function crossbond_event

type(event) function pol1bond_event (rij, vij, j, n)
    implicit none  
    ! ** calling variables ***********************************
    type(position), intent(in) :: rij 
    type(velocity), intent(in) :: vij 
    ...
    ! determine the event partner
    pol1bond_event%partner%one = j
    pol1bond_event%partner%two = n

    ! calculate quadratic parameters 
    aij = (vij%v(1) ** 2) + (vij%v(2) ** 2)
    bij = (rij%r(1) * vij%v(1)) + (rij%r(2) * vij%v(2))
    icij = (rij%r(1) ** 2) + (rij%r(2) ** 2) - (ipol1bond ** 2)
    ocij = (rij%r(1) ** 2) + (rij%r(2) ** 2) - (opol1bond ** 2)
    idiscr = (bij ** 2) - (aij * icij)
    odiscr = (bij ** 2) - (aij * ocij)
    ...
    if (bij < 0.) then ! the centers are approaching
        if (idiscr > 0.) then ! the centers will collide 
            pol1bond_event%type = 14 ! an event will occur at the inner bond length
            pol1bond_event%time = (-bij - sqrt(idiscr)) / aij
        else ! the repulsive centers will miss each other 
            pol1bond_event%type = 15 ! an event will take place at the outer bond length
            pol1bond_event%time = (-bij + sqrt(odiscr)) / aij
        end if 
    else ! the centers are receding
        pol1bond_event%type = 15 ! an event will occur at the outer bond length 
        pol1bond_event%time = (-bij + sqrt(odiscr)) / aij
    end if 
end function pol1bond_event

type(event) function pol2bond_event (rij, vij, j, n)
    implicit none  
    ! ** calling variables ***********************************
    type(position), intent(in) :: rij 
    type(velocity), intent(in) :: vij 
    ...
    ! determine the event partner
    pol2bond_event%partner%one = j
    pol2bond_event%partner%two = n

    ! calculate quadratic parameters 
    aij = (vij%v(1) ** 2) + (vij%v(2) ** 2)
    bij = (rij%r(1) * vij%v(1)) + (rij%r(2) * vij%v(2))
    icij = (rij%r(1) ** 2) + (rij%r(2) ** 2) - (ipol2bond ** 2)
    ocij = (rij%r(1) ** 2) + (rij%r(2) ** 2) - (opol2bond ** 2)
    idiscr = (bij ** 2) - (aij * icij)
    odiscr = (bij ** 2) - (aij * ocij)
    ...
    if (bij < 0.) then ! the centers are approaching
        if (idiscr > 0.) then ! the centers will collide 
            pol2bond_event%type = 16 ! an event will occur at the inner bond length
            pol2bond_event%time = (-bij - sqrt(idiscr)) / aij
        else ! the repulsive centers will miss each other 
            pol2bond_event%type = 17 ! an event will take place at the outer bond length
            pol2bond_event%time = (-bij + sqrt(odiscr)) / aij
        end if 
    else ! the centers are receding
        pol2bond_event%type = 17 ! an event will occur at the outer bond length 
        pol2bond_event%time = (-bij + sqrt(odiscr)) / aij
    end if 
end function pol2bond_event
...
type(event) function polsphere_event (rij, vij, j, n)
    ...
    integer, intent(in) :: n ! uplist particle of uplist group 
    ! ** local variables *************************************
    real(kind=dbl) :: aij, bij, cij_1, cij_2, cij_3, cij_4, cij_5 ! quadratic equation constants wrt to discontinuities 1, 2, 3, 4, and 5
    real(kind=dbl) :: discr1, discr2, discr3, discr4, discr5 ! discrimenant wrt cij_1, _2, _3, _4, and _5
	...
	if (bij < 0.0) then ! the centers are approaching
        ...
        else if (cij_5 < 0.0) then ! if rij is within the fourth well
            if (discr4 > 0.0) then ! if the cores will collide
                polsphere_event%type = 7 ! an event will take place at sigma4+
                polsphere_event%time = (-bij - sqrt(discr4)) / aij
            else ! the cores will miss 
                polsphere_event%type = 8 ! an event will take place at sigma5-
                polsphere_event%time = (-bij + sqrt(discr5)) / aij
            end if 
		else ! rij is outside the potential
			if (discr5 > 0.0) then ! if the cores will collide
                polsphere_event%type = 9 ! an event will take place at sigma5+
                polsphere_event%time = (-bij - sqrt(discr5)) / aij
...
subroutine collide (a, b, this_event, w)
    ...
    real(kind=dbl) :: bij, distance, bump, discr2, discr3, discr4, discr5, delep ! dot prroduct of moment and velocity vectors, distance between atoms
	...
	if (this_event%type == 1) then ! the event is at the repulsive core
        ...
        n_well = n_well + 1
	else if (this_event%type == 6) then ! the event is a collision at sigma 4-
        delep = epsilon4 - epsilon5
		if ((square(a%one)%circle(a%two)%pol * square(b%one)%circle(b%two)%pol) == -1) then ! if the spheres are attracted to one another
            ! determine if the spheres have enough energy required to dissociate
            discr4 = bij**2 - (4.*sg4sq*delep)
			if (discr4 >= 0) then ! if the pair meet the energy requirement
                ! the pair will dissociate
				do q = 1, ndim 
                    impulse(q) = - rij%r(q) * ((bij - sqrt(discr4)) / (2. * sg4sq))
				end do 
                bump = 1.0
			else ! the pair will remain associated ("bounce")
				do q = 1, ndim 
                    impulse(q) = - rij%r(q) * (bij / sg4sq)
				end do 
                bump = -1.0
			end if 
		else ! the spheres are repeled from one another
            ! the pair will dissociate
			do q = 1, ndim 
                impulse(q) = - rij%r(q) * ((bij - sqrt((bij ** 2) + (4. * sg4sq * delep))) / (2. * sg4sq)) ! CHECK THIS!
			end do 
            bump = 1.0
		end if
        n_well = n_well + 1
	else if (this_event%type == 7) then ! the event is a collision at sigma 4+
        delep = epsilon4 - epsilon5
		if ((square(a%one)%circle(a%two)%pol * square(b%one)%circle(b%two)%pol) == -1) then ! if the spheres are attracted to one another
            ! the pair will associate 
			do q = 1, ndim 
                impulse(q) = - rij%r(q) * ((bij + sqrt((bij ** 2) + (4. * sg4sq * delep))) / (2. * (sg4sq)))
			end do 
            bump = -1.0
		else ! the spheres are repeled from one another 
            ! determine if the spheres have enough energy required to associate 
            discr4 = (bij ** 2) - (4. * sg4sq * delep)
			if (discr4 >= 0) then ! the the pair meet the energy requirement
                ! the pair will associate
				do q = 1, ndim 
                    impulse(q) = - rij%r(q) * ((bij + sqrt(discr4)) /(2. * sg4sq)) ! CHECK THIS
				end do 
                bump = -1.0
			else ! the pair do not meet the energy requirement
                ! the pair will remain dissociated ("bounce")
				do q = 1, ndim 
                    impulse(q) = - rij%r(q) * (bij / sg4sq) ! CHECK THIS
				end do 
                bump = 1.0
			end if 
		end if
        n_well = n_well + 1
    else if (this_event%type == 8) then ! the event is a collision at sigma 5-
        delep = epsilon5
        if ((square(a%one)%circle(a%two)%pol * square(b%one)%circle(b%two)%pol) == -1) then ! if the spheres are attracted to one another
            ! determine if the spheres have enough energy required to dissociate
            discr5 = bij**2 - (4.*sg5sq*delep)
            if (discr5 >= 0) then ! if the pair meet the energy requirement
                ! the pair will dissociate
                do q = 1, ndim 
                    impulse(q) = - rij%r(q) * ((bij - sqrt(discr5)) / (2. * sg5sq))
                end do 
                bump = 1.0
            else ! the pair will remain associated ("bounce")
                do q = 1, ndim 
                    impulse(q) = - rij%r(q) * (bij / sg5sq)
                end do 
                bump = -1.0
            end if 
        else ! the spheres are repeled from one another
            ! the pair will dissociate
            do q = 1, ndim 
                impulse(q) = - rij%r(q) * ((bij - sqrt((bij ** 2) + (4. * sg5sq * delep))) / (2. * sg5sq)) ! CHECK THIS!
            end do 
            bump = 1.0
        end if
        n_well = n_well + 1
    else if (this_event%type == 9) then ! the event is a collision at sigma 5+
        delep = epsilon5
        if ((square(a%one)%circle(a%two)%pol * square(b%one)%circle(b%two)%pol) == -1) then ! if the spheres are attracted to one another
            ! the pair will associate 
            do q = 1, ndim 
                impulse(q) = - rij%r(q) * ((bij + sqrt((bij ** 2) + (4. * sg5sq * delep))) / (2. * (sg5sq)))
            end do 
            bump = -1.0
        else ! the spheres are repeled from one another 
            ! determine if the spheres have enough energy required to associate 
            discr5 = (bij ** 2) - (4. * sg5sq * delep)
            if (discr5 >= 0) then ! the the pair meet the energy requirement
                ! the pair will associate
                do q = 1, ndim 
                    impulse(q) = - rij%r(q) * ((bij + sqrt(discr5)) /(2. * sg5sq)) ! CHECK THIS
                end do 
                bump = -1.0
            else ! the pair do not meet the energy requirement
                ! the pair will remain dissociated ("bounce")
                do q = 1, ndim 
                    impulse(q) = - rij%r(q) * (bij / sg5sq) ! CHECK THIS
                end do 
                bump = 1.0
            end if 
        end if
        n_well = n_well + 1
	else if (this_event%type == 10) then ! the event is at the repulsive inner neighbor bond length
		do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / inbond**2)
		end do 
        bump = 1.0
        n_bond = n_bond + 1
	else if (this_event%type == 11) then ! the event is at the repulsive outer neighbor bond length
		do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / onbond**2)
		end do 
        bump = -1.0
        n_bond = n_bond + 1
	else if (this_event%type == 12) then ! the event is occuring at the repulsive inner cross bond length 
		do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / icbond**2)
		end do 
        bump = 1.0
        n_bond = n_bond + 1
	else if (this_event%type == 13) then ! the event is occuring at the repulsive outer cross bond length
		do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / ocbond**2)
		end do 
        bump = -1.0
        n_bond = n_bond + 1
    else if (this_event%type == 14) then ! the event is occuring at the repulsive inner first polarized bond
        do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / ipol1bond**2)
        end do 
        bump = -1.0
        n_bond = n_bond + 1
    else if (this_event%type == 15) then ! the event is occuring at the repulsive outer first polarized bond
        do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / opol1bond**2)
        end do 
        bump = -1.0
        n_bond = n_bond + 1
    else if (this_event%type == 16) then ! the event is occuring at the repulsive inner second polarized bond
        do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / ipol2bond**2)
        end do 
        bump = -1.0
        n_bond = n_bond + 1
    else if (this_event%type == 17) then ! the event is occuring at the repulsive outer second polarized bond
        do q = 1, ndim 
            impulse(q) = - rij%r(q) * (bij / opol2bond**2)
        end do 
        bump = -1.0
        n_bond = n_bond + 1
    end if 
